 ...
 * Purpose : The component for the search bar, can be reused
 */
...
const AddButton = (props) => {
  ...
  const addIngredientHelper = () => {
    const { addIngredient, getSuggestions } = bindActionCreators(
      allActions,
      dispatch
    );
  ...
  return (
    <button
      className={["button", "addButton"].join(" ")}
      onClick={addIngredientHelper}
    >
      Add
    </button>
  );
